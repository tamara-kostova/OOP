...
public:
    ...
    double voDolari(){
        if (strcmp(valuta, "USD")==0)
            return 1.0*iznos;
        return 1.0*iznos*Transakcija::usd/Transakcija::eur;
    }
    void pecati(){
...
public:
    ...
    void izvestajVoEvra() {
        ...
        cout<<"Korisnikot so smetka: "<<smetka<<" ima momentalno saldo od "<<vkupno<<" EUR"<<endl;
    }
    void izvestajVoDolari() {
        double vkupno = vkupnosaldo()/Transakcija::getusd();
        cout<<"Korisnikot so smetka: "<<smetka<<" ima momentalno saldo od "<<vkupno<<" USD"<<endl;
    }
